...
const ResourceDetailModal: React.FC<ResourceDetailModalProps> = ({ resource, onClose, platforms, categories }) => {
  ...
  const getGroupTitle = (categoryId: string) => {
    const category = categories.find(c => c.id === categoryId);
    return category ? category.group_title : 'Unknown';
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full" onClick={e => e.stopPropagation()}>
        <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
          <h2 className="text-2xl font-bold flex items-center">
            <i className={`${getFormatIcon(resource.format)} text-blue-500 mr-3`}></i>
            {resource.title}
          </h2>
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto">
          ...
          <div className="mb-4">
            {(Array.isArray(resource.category) ? resource.category : [resource.category]).map(catId => (
              <span key={catId} className="inline-block bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 text-xs px-2 py-1 rounded mr-2">{getGroupTitle(catId)}</span>
            ))}
          </div>
        ...
        <div className="p-6 flex justify-end gap-4 bg-gray-50 dark:bg-gray-700/50 rounded-b-lg">
